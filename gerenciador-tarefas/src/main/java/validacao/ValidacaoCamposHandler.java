...
public class ValidacaoCamposHandler {
    ...
    public ResponseEntity<ErrorResponse> tratarValidacoes(MethodArgumentNotValidException ex) {
        List<Map<String, String>> listaErros = ex.getBindingResult().getFieldErrors().stream().map(erro -> {
                    Map<String, String> erros = new HashMap<>();
                    erros.put("campo", obterNomePropriedades(erro));
    ...
    private String obterNomePropriedades(final FieldError error) {
